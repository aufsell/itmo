...
public class InputReader {
    ...
    public Coordinates receiveCoordinates() {
        return new Coordinates(0, receiveX(), receiveY());
    }
    ...
    public Long receivePrice() {
        for (; ; ) {
            try {
                ...
                if (priceString.isEmpty()) return 0L;
                else {
    ...
    public Float receiveDiscount() {
        for (; ; ) {
            try {
                ...
                if (discountString.isEmpty()) return 0F;
                else {
    ...
    public Ticket receiveTicket(
            final int id, final AuthUser authUser
    ) {
        final var venue = receiveVenue(id);

        return new Ticket(id, authUser.getId(), receiveName(), receiveCoordinates(),
                LocalDateTime.now(), receivePrice(), receiveDiscount(), receiveTicketType(), venue);
    }
}
