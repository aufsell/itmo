...
public abstract class AbstractClientChannel extends AbstractChannel {
     ...
     * @throws IOException ошибка при получении данных
     */
    public String sendCommandAndReceiveResponse(
            final AuthUser authUser,
            final String command, final Object... args
    ) throws IOException {
        AtomicReference<String> response = new AtomicReference<>(EMPTY_RESPONSE_MESSAGE);

        // ждем ответа от сервера!
        awaitPacket(new CommandPacket.Request(authUser, command, args), (packet) -> {
            if (packet instanceof CommandPacket.Response r)
                response.set(r.getResponse());
        });

        return response.get();
    }

    public LoginPacket.Response tryLoginAndReceiveResponse(
            final String user, final String password, final AuthAction authAction
    ) throws IOException {
        AtomicReference<LoginPacket.Response> response = new AtomicReference<>();

        awaitPacket(new LoginPacket.Request(user, password, authAction), (packet) -> {
            if (packet instanceof LoginPacket.Response r)
                response.set(r);
        });
    ...
    private byte[] receive() {
        ...
        while (true) {
            try {
                ...
                throw new IllegalStateException("Failed to receive data!", e);
            }
        }
